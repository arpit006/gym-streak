...
func createMigrate() *migrate.Migrate {
	...
	if err != nil {
		logger.Panic(fmt.Sprintf("Failed to prepare migration on [%s] database: [%s]. Error is - %s", dbConf.Type, dbConf.DatabaseName, err.Error()))
	}
...
func RunDatabaseMigrations() error {
	...
	if err != nil {
		...
		logger.Error(fmt.Sprintf("Migration failed. Error is - %s", err))
	}
...
func RollbackDatabaseMigrations() error {
	...
	if err != nil {
		...
		logger.Error(fmt.Sprintf("Rollback failed. Error is - %s", err))
	}
